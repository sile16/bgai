...
We're changing from a 6-way mutually exclusive outcome distribution to a 4-way independent output format with conditional gammon probabilities.

### Old Format (6-way, mutually exclusive)
```
[win, gammon_win, bg_win, loss, gammon_loss, bg_loss]
```
- Outputs are logits, converted to probabilities via softmax
- Probabilities sum to 1.0 (mutually exclusive outcomes)
- Loss: cross-entropy over the 6 outcomes

### New Format (4-way, independent with conditionals)
```
...
#### Remove old function:
```python
# DELETE: terminal_value_probs_from_reward (6-way version)
```
...
equity = probs_to_equity(value_probs)
...
Ensure networks can specify `value_head_out_size=4` and the framework handles sigmoid vs softmax appropriately.

Option A: Add a flag to the network/evaluator config:
```python
value_head_type: str = "4way_conditional"  # or "6way_categorical"
```

Option B: Infer from output size (4 = conditional, 6 = categorical)
...
- Delete old 6-way functions after bgai repo is updated
- The bgai repo will handle its own loss function using these primitives
