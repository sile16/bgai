...
class TestMetalBackgammonEnv:
    """Tests for PGX backgammon environment on Metal."""

    def test_backgammon_import(self):
        """Test that PGX backgammon can be imported."""
        try:
            from pgx import backgammon
            assert backgammon is not None
        except ImportError:
            pytest.skip("PGX backgammon not installed")

    def test_backgammon_init(self):
        """Test backgammon environment initialization."""
        try:
            from pgx import backgammon
        except ImportError:
            pytest.skip("PGX backgammon not installed")

        env = backgammon.Backgammon()
        ...
        assert hasattr(state, 'legal_action_mask')

    def test_backgammon_step(self):
        """Test backgammon environment step on Metal.

        This test verifies that PGX env.step() works correctly on Metal after
        the fix that replaced jax.lax.cond with jnp.where in core.py and
        backgammon.py. The original lax.cond calls failed on Metal with
        'Unable to serialize MPS module' when branches returned boolean values.
        """
        try:
            from pgx import backgammon
        except ImportError:
        ...
        # Get legal actions and take one
    ...
    def test_backgammon_batched_init(self):
        """Test batched backgammon initialization (vmap)."""
        try:
            from pgx import backgammon
        except ImportError:
            pytest.skip("PGX backgammon not installed")
